...
export const month = ['января', 'февраля','марта','апреля','мая','июня','июля',
'августа','сентября','октября','ноября','декабря' ]


const useCalendar = (daysShort = daysArr, monthNames = monthArr) => {
  ...
  const getEventMonth = (year, month, day) => {
    const date = today.setFullYear(year, month, day)
    const newDate = new Date(date)
    setSelectedDate(new Date(newDate.getFullYear(), newDate.getMonth()-1, newDate.getDate()))
  }
